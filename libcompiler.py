...
#Edit by minh12312 (adr_of [label][offset][base_address], program_info output (Program is end at,...), fill, def, var, multi-adr,...)
...
def load_defs(path):
	...
	while i < len(lines):
		...
		if line.strip().startswith('def ') and line.strip().endswith(':'):
			header = line.strip()[4:-1]
			name, raw_args = header.split('(')
			name = canonicalize(name)
			params = [canonicalize(x.strip()) for x in raw_args[:-1].split(',')]
			body = []
			i += 1
			while i < len(lines):
				next_line = lines[i]
				if next_line.startswith(' ') or next_line.startswith('\t'):
					body.append(canonicalize(next_line.strip()))
					i += 1
				else:
					break
			def_cmds[name] = (params, body)
		else:
			i += 1
	return def_cmds
def process(line):
    # the processing result will affect those variables
    global result, labels, adr_of_cmds, adr_arith_cmds, pr_length_cmds, home, string_vars, in_comment, endaddr, end
    ...
    if in_comment:
        ...
        '''
        cmd <new_cmd> = old_cmd
        Create alias for a cmd
        '''
        parts = re.match(r'^\s*(\w+)\s+(\w+)\s*=\s*(.+)$', line)
        assert parts, f'Invalid cmd assign syntax: {line}'
        old_cmd=parts.group(3)
        new_cmd=parts.group(2)
    ...
    elif line.startswith('gadget') and '=' in line:
        '''
        gadget <gadget_name> = <gadget>
        Create a gadget_name from address
        '''
        parts = re.match(r'^\s*(\w+)\s+(\w+)\s*=\s*(.+)$', line)
        assert parts, f'Invalid gadget assign syntax: {line}'
        addr=int(parts.group(3),16)
        cmd = canonicalize(parts.group(2))
        add_command(commands, addr, cmd, tags=('rt',), debug_info='manual gadget')
    elif line.startswith('hex') and '=' in line:
        '''
        Syntax:
            hex <hex_name> = <hex_value>
            hex <hex_name> = 0x<hex_value>
        Create a alias for hex
        '''
        parts = re.match(r'^\s*(\w+)\s+(\w+)\s*=\s*(.+)$', line)
        assert parts, f'Invalid hex assign syntax: {line}'
        name, hexstr = parts.group(2), parts.group(3).strip()
        checkhex=False
        ...
        assert len(hexstr) % 2 == 0, f'Invalid data length'
        bytes_list = [int(hexstr[i:i+2], 16) for i in range(0, len(hexstr), 2)]
        if checkhex:
            bytes_list.reverse()
        value = 0
        for b in bytes_list:
            value = (value << 8) | b
        datalabels[canonicalize(name)] = value
...
"""
        ...
        for i in huh:
            process(i)
    elif line.startswith('size(') and line.endswith(')'):
        '''
        (Editing from adr_adrith)
        Syntax: size(label2 - label1)
            size(label2[+offset] - label1[-offset])
        Calculate label2 - label1 + offset2 - offset1
        '''
        ...
        for i in range(len(inner)):
            if inner[i] == '[':
                ...
                break
        if split_pos == -1:
            raise ValueError(f'Invalid size() syntax: {line}')
        left_raw = inner[:split_pos].strip()
        right_raw = inner[split_pos + 1:].strip()
        def parse_label_offset(expr):
            if '[' in expr and ']' in expr:
                name = expr[:expr.index('[')].strip()
                offset_str = expr[expr.index('[')+1 : expr.index(']')].strip()
                try:
                    offset = int(offset_str, 0)
                except:
                    raise ValueError(f'Invalid offset in: {expr}')
            else:
                name = expr.strip()
                offset = 0
            return offset, name
        left_offset, left_label = parse_label_offset(left_raw)
        right_offset, right_label = parse_label_offset(right_raw)
        ...
        name = line[:line.find('(')]
        if name in def_cmds:
            vals = [canonicalize(x) for x in line[line.find('(')+1 : line.rfind(')')].split(',')]
            params, body = def_cmds[name]
            assert len(vals) == len(params), f'The number of parameters passed to the function is invalid.'
            for b in body:
                for a, v in zip(params, vals):
                    b = re.sub(r'\b{}\b'.format(re.escape(a)), v, b)
                process(b)
...
def process_program(args, program, overflow_initial_sp):
    ...
    while i < len(modified_program):
        line = modified_program[i]
        if line.strip().startswith('def ') and line.strip().endswith(':'):
            head = line.strip()[4:-1]
            name, raw_args = head.split('(')
            name = canonicalize(name)
            params = [canonicalize(x) for x in raw_args[:-1].split(',')]
            body = []
            i += 1
            while i < len(modified_program):
                l = modified_program[i]
                if l.startswith(' ') or l.startswith('\t'):
                    body.append(canonicalize(l.strip()))
                    i += 1
                else:
                    break
            prog_defs[name] = (params, body)
            continue
        i += 1
    def_cmds = {**def_cmds, **prog_defs} 
    found_org = any('org' in canonicalize(del_inline_comment(line)).lower() for line in modified_program)
    if not found_org:
        raise ValueError("Program must contain 'org <address>'.")
    d=0
    org_found=False
...
def print_output(args):
        ...
        print(Fore.GREEN + f'Program at 0x{home:04X}:')
        ...
        print(Fore.CYAN + f'Program length (hex): 0x{len(result):04X} bytes')
